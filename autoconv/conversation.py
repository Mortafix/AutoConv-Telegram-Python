...
class Conversation:
	...
	def add_states(self,states:Union[Sequence[State],State]):
	...
	def add_routes(self,state:State,routes:Optional[dict]=None,default:Optional[State]=None,back:Optional[State]=None):
		...
		if state not in self.state_list: self.add_states(state)
		if routes:
			for k,v in routes.items():
				...
				if v not in self.state_list: self.add_states(v)
			self.routes.update({state.name:routes})
		if default:
			if default not in self.state_list: self.add_states(default)
			s.update({-1:default}) if (s := self.routes.get(state.name)) else self.routes.update({state.name:{-1:default}})
		if back:
			if back not in self.state_list: self.add_states(back)
